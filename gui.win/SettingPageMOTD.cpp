    ...
    if(uMsg == WM_COMMAND) {
        if(LOWORD(wParam) == EDT_MOTD) {
            if(HIWORD(wParam) == EN_CHANGE) {
                ...
                if(buf == NULL) {
                    AppendDebugLogFormat("[MEM] Cannot allocate %" PRIu64 " bytes for buf in SettingPageMOTD::PageMOTDProc\n", (uint64_t)(iLen+1));
                    return 0;
                }
                ...
                if(HeapFree(clsServerManager::hPtokaXHeap, HEAP_NO_SERIALIZE, (void *)buf) == 0) {
					AppendDebugLogFormat("[MEM] Cannot deallocate buf in SettingPageMOTD::PageMOTDProc\n");
                }
...
void SettingPageMOTD::Save() {
    ...
    if(buf == NULL) {
        AppendDebugLogFormat("[MEM] Cannot allocate %" PRIu64 " bytes for buf in SettingPageMOTD::Save\n", (uint64_t)(iAllocLen+1));
        return;
    }
    ...
    if(HeapFree(clsServerManager::hPtokaXHeap, HEAP_NO_SERIALIZE, (void *)buf) == 0) {
		AppendDebugLogFormat("[MEM] Cannot deallocate buf in SettingPageMOTD::Save\n");
    }
